...
from escrimeBlois.form import FormInscription, FormInscriptionEvent
import os
...
@app.route('/evenement/inscription/<int:id_evenement>', methods=['GET', 'POST'])
def inscription_event(id_evenement):
    ...
    if current_user.is_authenticated:
        ...
        deja_inscrit = Inscription.query.filter_by(id_evenement=id_evenement, nom=current_user.nom_personne, prenom=current_user.prenom_personne).first()
        if deja_inscrit:
...
def admin_inscription_evenement():
    ...
    return render_template(
        "admin_gestion_inscription_evenement.html", inscriptions=inscriptions
    )
...
def admin_inscription_evenement_view(id_inscription):
    ...
    return render_template(
        "admin_gestion_inscription_evenement_view.html", inscription=inscription
    )
        ...
        for fichier in fichiers:
            if fichier and fichier.filename != "":
                chemin_fichier = os.path.join(
                    app.config["UPLOAD_FOLDER"], fichier.filename
                )
...
def change_password():
    ...
    if not any(c.isalpha() for c in new_password) or not any(
        ...
        flash(
            "Le mot de passe doit contenir au moins une lettre et un chiffre.", "error"
        )
        return redirect(url_for("infos_persos"))
    ...
    try:
        ...
        db.session.rollback()
        flash(
            "Une erreur est survenue lors de la mise à jour du mot de passe.", "error"
        )
